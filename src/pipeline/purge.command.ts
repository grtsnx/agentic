...
// It also removes the local output/agents.config.json and blanks the auto-generated
// IDs in .env (ANTHROPIC_VAULT_ID always; ANTHROPIC_ENVIRONMENT_ID with
// --with-environments; ANTHROPIC_MEMORY_STORE_ID with --with-memory) so no stale
// IDs are reused — the next `setup:pipeline` re-provisions and re-fills them.
//
...
import { clearEnvVars } from './lib/env-file';
...
function clearLocalConfig(): void {
  ...
  if (fs.existsSync(configPath)) {
    ...
    console.log(
      `   ✓ removed local ${path.relative(process.cwd(), configPath)}`,
    );
  }
}

// Blank the auto-generated IDs in .env for whatever resources were purged, so the
// next `setup:pipeline` provisions and re-fills them instead of reusing dead IDs.
function clearAutoGeneratedEnv(
  withEnvironments: boolean,
  withMemory: boolean,
): void {
  const keys = ['ANTHROPIC_VAULT_ID']; // vaults are always purged
  if (withEnvironments) keys.push('ANTHROPIC_ENVIRONMENT_ID');
  if (withMemory) keys.push('ANTHROPIC_MEMORY_STORE_ID');

  const { cleared, path: envPath } = clearEnvVars(keys);
  if (cleared.length > 0) {
    console.log(
      `   ✓ cleared ${cleared.join(', ')} in ${path.relative(process.cwd(), envPath)}`,
    );
  }
}
...
async function main(): Promise<void> {
  ...
  if (withEnvironments) {
    ...
    console.log(
      '   • environments: left intact (pass --with-environments to wipe)',
    );
  }
  if (withMemory) {
  ...
  clearLocalConfig();
  clearAutoGeneratedEnv(withEnvironments, withMemory);
